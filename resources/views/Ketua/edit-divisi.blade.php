          ...
          <div class="section-body">
            <div class="row mt-sm-4">
              <div class="col-12 col-md-12 col-lg-12">
                <div class="card">
                  <form method="POST" action="{{route('updateDivisi.ketua',[$divisi->divisiId])}}" enctype="multipart/form-data">
                    @csrf
                    <div class="card-header text-left">
                        <h4>Edit Divisi</h4>
                        <a href="{{route('deleteDivisi.ketua',[$divisi->divisiId])}}" class="btn btn-danger">Hapus divisi</a>
                    </div>
                    <div class="card-body">
                      <div class="row">
                        ...
                        <div class="form-group col-6">
                          ...
                          <select class="form-control selectric" name="ketua">
                            @foreach($listAnggota as $anggota)
                              @if($anggota->statusacc==1)
                                @if($anggota->anggotaId==$divisi->ketuaId)
                                  <option value="{{$anggota->anggotaId}}" selected>{{$anggota->nama}}</option>
                                @endif
                                  <option value="{{$anggota->anggotaId}}">{{$anggota->nama}}</option>
                              @endif
                            @endforeach
                          </select>
                        </div>
                        <div class="form-group col-12">
                          ...
                          <input type="textarea" class="form-control" name="desc" value="{{$divisi->deskripsi}}">
                        </div>
                        <div class="form-group col-12">
